...
#-- Simple linear regression (knots = 0) --#
# Generate model
...
plot_prescriptions_simple <- ggplot(data = fitted_prescriptions_simple) +
    ...
    labs(title = paste0('Simple (AICc =  ', round(AICc(prescriptions_simple), 2), ')'),
         ...
         x = 'Date') +
    scale_y_continuous(limits = c(48, 70)) +
...
#-- Interrupted linear regression (knots = 1) --#
# Generate model
prescriptions_spline <- lm(prescriptions_total ~ lspline(x = month,
                                                         knots = 25,
                                                         marginal = FALSE), 
                           data = data)
...
#-- Simple linear regression (knots = 0): parallel slopes --#
# Generate model
...
# Inspect model coefficients
tidy(prescriptions_parallel, conf.int = TRUE)
...
# Calculate difference in AIC (vs simple regression, the simplest model)
## Values < 2 indicate no meaningful information loss (Burnham & Anderson 2004)
prescription_delta_aic0 <- AICc(prescriptions_parallel) - AICc(prescriptions_simple)
prescription_delta_aic0
prescription_delta_aic1 <- AICc(prescriptions_spline) - AICc(prescriptions_simple)
prescription_delta_aic1
prescription_delta_aic2 <- AICc(prescriptions_simple) - AICc(prescriptions_simple)
...
# Get data
vec_rl <- c(rl2, rl1, rl0)

vec_names <- c('simple v simple', 'simple v spline', 'simple vs parallel')
...
plot_pills_simple <- ggplot(data = fitted_pills_simple) +
    ...
    scale_fill_tableau() +
    scale_colour_tableau() +
    theme(plot.margin = margin(t = 5.5, r = 50, b = 5.5, l = 20)); plot_pills_simple

#-- Interrupted linear regression (knots = 1) --#
# Generate model
pills_spline <- lm(pills_total ~ lspline(x = month,
                                         knots = 25,
                                         marginal = FALSE), 
                   data = data)
...
plot_pills_spline <- ggplot(data = fitted_pills_spline) +
    ...
    scale_fill_tableau() +
    scale_colour_tableau() +
    theme(plot.margin = margin(t = 5.5, r = 50, b = 5.5, l = 20)); plot_pills_spline 
...
# Calculate difference in AIC (vs simple linear regression, the simplest model)
## Values < 2 indicate no meaningful information loss (Burnham & Anderson 2004)

pills_delta_aic0 <- AICc(pills_spline) - AICc(pills_simple)
pills_delta_aic0
pills_delta_aic1 <- AICc(pills_parallel) - AICc(pills_simple)
pills_delta_aic1
pills_delta_aic2 <- AICc(pills_simple) - AICc(pills_simple)
pills_delta_aic2
...
vec_names <- c('simple v simple', 'simple vs parallel', 'simple v spline')
...
plot_quantity_simple <- ggplot(data = fitted_quantity_simple) +
    ...
    labs(title = paste0('Simple (AICc =  ', round(AICc(quantity_simple), 2), ')'),
         y = expression('Total dose dispensed per month (10'^10*' mg)'),
         x = 'Date') +
    scale_y_continuous(limits = c(1.5, 1.9)) +
    scale_x_continuous(breaks = c(1, 13, 25, 37, 49),
                       labels = c('April 2017', 'April 2018', 
                                  'April 2019', 
                                  'April 2020', 'April 2021')) +
    scale_fill_tableau() +
    scale_colour_tableau() +
    theme(plot.margin = margin(t = 5.5, r = 50, b = 5.5, l = 20)); plot_quantity_simple

#-- Interrupted linear regression (knots = 1) --#
# Generate model
quantity_spline <- lm(dose_total ~ lspline(x = month,
                                           knots = 25,
                                           marginal = FALSE), 
                      data = data)
...
plot_quantity_spline <- ggplot(data = fitted_quantity_spline) +
    ...
    scale_fill_tableau() +
    scale_colour_tableau() +
    theme(plot.margin = margin(t = 5.5, r = 50, b = 5.5, l = 20)); plot_quantity_spline 
...
quantity_delta_aic0 <- AICc(quantity_spline) - AICc(quantity_simple)
quantity_delta_aic0
quantity_delta_aic1 <- AICc(quantity_parallel) - AICc(quantity_simple)
quantity_delta_aic1
quantity_delta_aic2 <- AICc(quantity_simple) - AICc(quantity_simple)
quantity_delta_aic2
...
vec_names <- c('simple v simple', 'simple v parallel', 'simple vs spline')
...
#-- Interrupted linear regression (knots = 1) --#
# Generate model
dose_spline <- lm(dose_per_prescription_total ~ lspline(x = month,
                                                        knots = 25,
                                                        marginal = FALSE), 
                  data = data)
...
dose_delta_aic0 <- AICc(dose_simple) - AICc(dose_parallel)
dose_delta_aic0
dose_delta_aic1 <- AICc(dose_spline) - AICc(dose_parallel)
dose_delta_aic1
dose_delta_aic2 <- AICc(dose_parallel) - AICc(dose_parallel)
dose_delta_aic2
