############################################################
#                                                          #
#              ARIMA modelling for gabapentin              #
#                                                          #
############################################################
...
head(gabapentin$date, 1)

tail(gabapentin$date, 1)
...
mod_dose_auto2 <- auto.arima(y = ts_dose_pre,
                             ...
                             stepwise = FALSE,
                             trace = TRUE,
                             method = 'CSS') # Change to 'CSS' otherwise residuals are autocorrelated
...
plot_prescriptions <- ggplot(data = data_5) +
    ...
    geom_vline(xintercept = 25,
               linetype = 2) +
    labs(title = expression('ARIMA(0,0,0) with non-zero mean'),
...
plot_dose <- ggplot(data = data_5) +
    ...
    geom_vline(xintercept = 25,
               linetype = 2) +
    labs(title = expression('ARIMA(0,1,0)(0,1,0)'[12]),
...
plot_patchwork <- plot_prescriptions_2 + plot_dose_2 + plot_layout(ncol = 2)

ggsave(plot = plot_patchwork,
       filename = 'figures/gabapentin_arima.png',
       height = 6,
       width = 16)
